...
import {onSubmitPhone, onErrorInputPhone, onCodeSuccess} from './authorize-form';
...
const successCallback = (e) => {
  // Колбек при успешной валидации формы в футере при попытке её отправки
  e.preventDefault();
  if (e.target.closest('.footer-form')) {
    e.target.closest('.footer-form').classList.add('footer-form--successed');
    setTimeout(() => {
      e.target.closest('.footer-form').classList.remove('footer-form--successed');
    }, 3000);
  }
};
...
const callbacks = {
  ...
  formFooterSuccessCallback: {
    validationSuccessCallback: successCallback,
  },
  authorizePhoneSubmit: {
    validationSuccessCallback: onSubmitPhone,
    validationErrorCallback: onErrorInputPhone,
  },
  authorizeCodeSubmit: {
    validationSuccessCallback: onCodeSuccess,
  },
};
