...
const hideError = (el) => {
  if (el.classList.contains('toggle-group')) {
    ...
    const errorEl = parent.querySelectorAll('[class^="custom-input__error"]');
    const errorSelectEl = parent.querySelector('.custom-select__error');
    el.setAttribute('aria-invalid', 'false');
    parent.classList.remove('is-invalid');
    parent.classList.add('is-valid');
    if (errorEl.length) {
      errorEl.forEach((elem) => {
        elem.style.maxHeight = null;
      });
    } else if (errorSelectEl) {
      errorSelectEl.style.maxHeight = null;
    }
  }
};
...
const showError = (el) => {
  if (el.classList.contains('toggle-group')) {
    ...
    const errorEl = parent.querySelector('.custom-input__error');
    const errorElEmpty = parent.querySelector('.custom-input__error-empty');
    const errorSelectEl = parent.querySelector('.custom-select__error');
    el.setAttribute('aria-invalid', 'true');
    parent.classList.add('is-invalid');
    parent.classList.remove('is-valid');
    if (parent.dataset.validateType === 'phone' && errorElEmpty && input.value.length <= BASE_COUNTRY_CODE.length) {
      errorEl.style.maxHeight = null;
      errorElEmpty.style.maxHeight = errorElEmpty.scrollHeight + 'px';
    } else if (errorEl) {
      if (errorElEmpty) {
        errorElEmpty.style.maxHeight = null;
      }
      errorEl.style.maxHeight = errorEl.scrollHeight + 'px';
    } else if (errorSelectEl) {
      errorSelectEl.style.maxHeight = errorSelectEl.scrollHeight + 'px';
    }
  }
};
